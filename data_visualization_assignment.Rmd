...
ggplot(dt, aes(x = Liver)) + 
  ...
  labs(x = "Genes", y = "Expression in TPM")

ggplot(dt, aes(x = `Breast - Mammary Tissue`)) + 
  geom_histogram(breaks = seq(0, 100, 2), position = "stack") + 
  labs(x = "Genes", y = "Expression in TPM")

# The distribution seems to scew towards the left around zero, this is reflective of the fact that most genes are not or expressed or expressed very low levels in the liver. I do not believe this is specific to the liver since a lot of genes have specialized functions and would not be expressed everywhere. I verified this by looking at the histogram for expression in other tissues and my hypothesis checked out. 
...
```{r q2}
library(patchwork)
p1 <- ggplot(data = dt, aes(x = Liver)) + 
  geom_density()+ 
  labs(x = "Genes", y = "Expression in TPM")
p2 <- ggplot(data = dt, aes(x = (Liver + 1))) + 
  geom_density()+ 
  scale_x_log10()+ 
  labs(x = "Genes", y = "Expression in TPM + pseudocount of 1")
p1 + p2
```
...
ggplot(data = dt, aes(x = (Liver + 1), y = (`Heart - Left Ventricle` + 1))) +
  ...
  geom_abline(slope = 1, ity = "dashed", color = "red")+
  scale_x_log10()+ 
  scale_y_log10()+
  labs(x = "Gene expression in the liver + pseudocount of 1", y = "Gene expression in the left ventricle + pseudocount of 1")
...
```{r q4}
mitochondrial_genes <- str_starts(dt$Description, pattern = "MT")
dt <- dt %>%
  mutate(is_mitochondrial = mitochondrial_genes)
head(dt)
ggplot(data = dt, aes(x = (Liver + 1), y = (`Heart - Left Ventricle` + 1), colour = is_mitochondrial)) +
  geom_point(size=1, alpha=0.2) +
  geom_abline(slope = 1, ity = "dashed", color = "red")+
  scale_x_log10()+ 
  scale_y_log10()+
  labs(x = "Gene expression in the liver + pseudocount of 1", y = "Gene expression in the left ventricle + pseudocount of 1")+
  scale_color_manual(values = c("FALSE" = "darkorange2", "TRUE" = "dodgerblue3"))
